...
class TestInter:
    ...
    def test_inter_on_executes(self):
        x = torch.rand(2, 2, 1)
        x2 = torch.rand(1, 2, 2)
        assert Inter.std(x, x2).shape == torch.Size([2, 2, 2])

    def test_inter_on_executes_with_factory(self):
        x = torch.rand(2, 2, 1)
        x2 = torch.rand(1, 2, 2)
        assert Inter.bounded.f()(x, x2).shape == torch.Size([2, 2, 2])
...
class TestUnion:
    ...
    def test_union_executes(self):
        x = torch.rand(2, 2, 1)
        x2 = torch.rand(1, 2, 2)
        assert Union.std(x, x2).shape == torch.Size([2, 2, 2])

    def test_union_executes_with_factory(self):
        x = torch.rand(2, 2, 1)
        x2 = torch.rand(1, 2, 2)
        print(Inter.std)
        assert Union.bounded.f()(x, x2).shape == torch.Size([2, 2, 2])

class TestInterOn:

    def test_inter_on_executes(self):
        x = torch.rand(2, 2, 1)
        assert InterOn.std(x, dim=-2, keepdim=False).shape == torch.Size([2, 1])

    def test_inter_on_executes_with_factory(self):
        x = torch.rand(2, 2, 1)
        assert InterOn.bounded.f()(x, dim=-2, keepdim=True).shape == torch.Size([2, 1, 1])
...
class TestUnionOn:
    ...
    def test_union_on_executes(self):
        x = torch.rand(2, 2, 1)
        assert UnionOn.std(x, dim=-2, keepdim=True).shape == torch.Size([2, 1, 1])

    def test_union_on_executes_with_factory(self):
        x = torch.rand(2, 2, 1)
        print(Inter.std)
        assert UnionOn.bounded.f()(x, dim=-2, keepdim=False).shape == torch.Size([2, 1])
